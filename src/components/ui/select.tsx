...
const SelectTrigger = React.forwardRef<React.ElementRef<typeof SelectPrimitive.Trigger>, React.ComponentPropsWithoutRef<typeof SelectPrimitive.Trigger>>(
  ({ className, children, ...props }, ref) => (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        'flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground ring-1 ring-foreground placeholder:text-muted-foreground focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1',
        className
    ...
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          ...
          className,
          'border-foreground bg-background/50 text-black  ring-white dark:text-white'
        )}
        position={position}
        {...props}
      >
  ...
  ({ className, children, ...props }, ref) => (
    <SelectPrimitive.Item
      ...
      className={cn(
        ...
        className,
        'hover:bg-white focus:bg-white hover:dark:bg-[#4d4d4d]  focus:dark:bg-[#4d4d4d] '
      )}
      {...props}
    >
